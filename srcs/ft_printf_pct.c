...
/*   Updated: 2018/02/16 16:30:27 by kgricour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int        ft_printf_pct(char **new_str, char *opt, int i)
{
	...
	if (ft_strchr(opt, '-') != NULL && new_str[0][i + j] == '%')
	{
		...
		len = ft_strlen(tmp2);
		tmp2 = ft_strjoin(ft_strsub(*new_str, 0 , i, 0),ft_strdup(tmp2));
	}
	else if (ft_strchr(opt, '-') == NULL && new_str[0][i + j] == '%' && noopt == 0)
	{
		...
		len = ft_strlen(tmp2);
		tmp2 = ft_strjoin(ft_strsub(*new_str, 0 , i, 0), ft_strdup(tmp2 + 1));
	}
	else
	{
		tmp2 = ft_strdup("%");
		ft_add_space(&tmp2, opt);
		len = ft_strlen(tmp2);
		tmp2 = ft_strjoin(ft_strsub(*new_str, 0 , i, 0), tmp2);
	}
